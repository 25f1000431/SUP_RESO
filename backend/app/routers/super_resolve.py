...
async def super_resolve_area(request: AOIRequest):
    ...
    try:
        download_info = download_sentinel2_aoi(request, input_path)

        arr, profile, transform, crs, raw_stats, normalized_stats, normalization_applied = (
            read_rgbn(input_path)
        )

        original_preview = make_rgb_preview(arr, is_original=True)
        ...
        return {
            ...
            "sentinel2": {
                ...
                "max_cloud_coverage": download_info["max_cloud_coverage"],
                "crs": str(crs),
            },
            "output": {
        ...
        print(f"[ERROR] super-resolve-area failed: {exc!r}")
        raise HTTPException(status_code=500, detail=f"Super-resolution failed: {exc}")
    finally:
